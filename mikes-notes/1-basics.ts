 ...
 */
x = 42; // 🚨 ERROR: x is of type string (hover over x to see)
...
/**
 * (4.1) foo() only accepts parameter of type "hello mars".
 * When we pass y of type "hello world" - we get an error.
 */
function foo(arg: "hello mars") { }
foo(y); // 🚨 ERROR

/**
 * (4.2) cons with objects. The type of foo within the object ends up to be a string because it's mutable.
*/
 ...
 */
let zz: number; //this is a type annotation
zz = 41;
...
// or ts can make assumption of the type if the arrau contains one element
let ab = ["look"];

/**
 * (9) we can even define a tuple - array which has a fixed length
 */
...
//You dont want to use array methods like push and unshift() on tuples because you can mess up types and TS will regard it as valid. In tuples - order and number of elems matters.

bb.push(1, 2, 3, 4)// valid but incorrect

/**
 * (10) Tuple values often require type annotations (  : [number, number] )
 */
const xx = [32, 31]; // number[] - array of numbers - not a tuple
const yy: [number, number] = [32, 31]; //tuple
...
// half a time contactInfo will have name&number, another half - name&email
...
contactInfo.name; // NOTE: we can only access the .name property  (the stuff HasPhoneNumber and HasEmail have in common) because it's guaranteed(!) that's it's gona be there! 
